...
func updateNode(_ node: SCNNode, fromDict dict: [String: Any], forDevice device: MTLDevice?) {
    ...
    // [추가된 부분] 빌보드(항상 카메라 보기) 기능 적용
    // Dart에서 isBillboard: true를 보내면 여기서 받아서 처리합니다.
    if let isBillboard = dict["isBillboard"] as? Bool, isBillboard {
        let billboardConstraint = SCNBillboardConstraint()
        billboardConstraint.freeAxes = .all // 상하좌우 모든 방향으로 회전
